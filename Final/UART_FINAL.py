...
def uart(port, baudrate, enteredtext, timeout=1):
    try:
        ser = serial.Serial(port, baudrate, timeout=.04)
        print(f"serial port @ {port} with baudrate {baudrate}")
        outputvals = []
        data_to_send = (enteredtext[0] + 32).to_bytes(1, "big") 
        #clear = ser.readline()
        #print(clear)
        #time.sleep(1)
        ...
        while True:
            ...
            data_received = int.from_bytes(ser.readline(), "big") + 1
            if data_received:
                print(f"rx: {data_received - 1}")
                outputvals.append(data_received - 1)
                if type(enteredtext[len(outputvals)]) == int:
                    data_to_send = (enteredtext[len(outputvals)] + 32).to_bytes(1, "big")
                else:
